...
#[command(
    ...
    about = "A CLI program for adding .js to import/export path in Typescript project"
)]
struct Args {
    #[command(subcommand)]
    cmd: Option<Command>,
...
enum Command {
    #[command(
        name = "addjs",
        about = "Add .js to import/export path in Typescript project"
    )]
    AddJs(RunArgs),
}
...
#[derive(Parser)]
struct RunArgs {
    #[clap(help = "The pattern to match files")]
    input: String,
}
...
fn main() {
    ...
    match args.cmd {
        Some(Command::AddJs(run_args)) => {
            if let Err(err) = run(&run_args.input) {
