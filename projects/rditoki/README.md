...
`reditoki` is an incomplete, idiomatic implementation of a
...
**Disclaimer** Please don't use reditoki in production. This project is
...
```
RUST_LOG=debug cargo run --bin reditoki-server
```
...
```
cargo run --bin reditoki set foo bar

cargo run --bin reditoki get foo
```
...
```
RUST_LOG=debug cargo run --bin reditoki-server --features otel
```
...
`reditoki` currently supports the following commands.
...
Contributions to `reditoki` are welcome. Keep in mind, the goal of the project
...
for inclusion in `reditoki` by you, shall be licensed as MIT, without any
